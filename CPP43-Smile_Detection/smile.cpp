...
//#include "opencv2/nonfree/nonfree.hpp" //SurfDescriptorExtractor
...
int main(int argc, const char** argv)
{
	...
	cascadeName = "haarcascade_frontalface_default.xml";//parser.get<string>("cascade");
	nestedCascadeName = "haarcascade_smile.xml";//parser.get<string>("smile-cascade");
	...
	double scale, bool tryflip)
{
	...
	for (size_t i = 0; i < faces.size(); i++)
	{
		...
		rectangle(img, cv::Point(0, img.rows), cv::Point(img.cols / 10, img.rows - rect_height), col, -1);
	}
